...
class Topic{
	...
	public function getByCategory($category_id){
		$this->db->query("SELECT topics.*, categories.*, topics.id, users.username, users.avatar FROM topics
						INNER JOIN categories
						ON topics.category_id = categories.id
						INNER JOIN users
						ON topics.user_id=users.id
						WHERE topics.category_id = :category_id	 ORDER BY topics.create_date DESC		
		");
		$this->db->bind(':category_id', $_GET['category']);
	...
	//	 Get  # of a new Topic  - This code is necessary to be able to delete unwanted topics without effecting the id of new coming topics
	...
	//  Get Topic Replies
		...
		//Insert Query
		$this->db->query("INSERT INTO topics (id, category_id, user_id, title, body,last_activity)
											VALUES (:newtopicid, :category_id, :user_id, :title,:body,:last_activity)");
		//Bind Values
		$this->db->bind(':newtopicid', $data['newtopicid']);
